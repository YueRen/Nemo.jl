...
@testset "FqPolyRepFieldElem.conformance_tests" begin
   test_Field_interface_recursive(Native.FiniteField(ZZRingElem(7), 5, "z")[1])
...
@testset "FqPolyRepFieldElem.constructors" begin
   R, x = Native.FiniteField(ZZRingElem(7), 5, "x")
   ...
   @test Native.FiniteField(ZZRingElem(7), 5)[1] isa FqPolyRepField
   @test Native.FiniteField(ZZRingElem(7), 5, :x)[1] isa FqPolyRepField
...
@testset "FqPolyRepFieldElem.rand" begin
   R, x = Native.FiniteField(ZZRingElem(17), 3, "x")
...
@testset "FqPolyRepFieldElem.iteration" begin
   for n = [2, 3, 5, 13, 31]
      R, _ = Native.FiniteField(ZZRingElem(n), 1, "x")
      elts = Nemo.AbstractAlgebra.test_iterate(R)
      @test elts == R.(0:n-1)
      R, _ = Native.FiniteField(ZZRingElem(n), rand(2:9), "x")
      Nemo.AbstractAlgebra.test_iterate(R)
   end
end

@testset "FqPolyRepFieldElem.lift" begin
   R, x = Native.FiniteField(ZZ(23), 2, "x")
   f = 8x + 9
